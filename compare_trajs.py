...
if test_var == "wilcoxon":
    dicti_wilcoxon_traj = {"variable": [], "ws": [], "model1": [], "model2": [], "u long": [], "p long": [], "u lat": [], "p lat": []}
        ...
        for ws_long in ws_long_list:
            ...
            for model1_ix in range(len(model_list)):
                for model2_ix in range(model1_ix + 1, len(model_list)):
                    ...
                    try:
                        uval1, pval1 = stats.wilcoxon(xmodel1, xmodel2)
                    except:
                        uval1, pval1 = 1.0, 1.0
                    try:
                        uval2, pval2 = stats.wilcoxon(ymodel1, ymodel2)
                    except:
                        uval2, pval2 = 1.0, 1.0
                    print(var, ws_long.split("_")[0], model_list[model1_ix], model_list[model2_ix], uval1, pval1, uval2, pval2)
                    dicti_wilcoxon_traj["variable"].append(var)
                    dicti_wilcoxon_traj["ws"].append(ws_long.split("_")[0])
                    dicti_wilcoxon_traj["model1"].append(model_list[model1_ix])
                    dicti_wilcoxon_traj["model2"].append(model_list[model2_ix])
                    dicti_wilcoxon_traj["u long"].append(uval1)
                    dicti_wilcoxon_traj["p long"].append(pval1)
                    dicti_wilcoxon_traj["u lat"].append(uval2)
                    dicti_wilcoxon_traj["p lat"].append(pval2)

    df_write = pd.DataFrame(dicti_wilcoxon_traj)
    df_write.to_csv("dicti_wilcoxon_traj.csv", index = False)
