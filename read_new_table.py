...
color_for_model = {"Att": "#00ff00", "GRU": "#ff0000", "LSTM": "#0000ff", "RNN": "#00ffff", "UniTS": "#ff00ff"}
    ...
    for metric in used_metric:
        for plttype in ["all", "best"]:
            ...
            for var in dictio:
                ...
                for model in set_of_models[plttype]:
                    arr_model = []
                    for ws in sorted_ws:
                ...
                varnew = var.replace("_", " ").replace("longitude no abs", "$x$ offset")
            ...
            for label_one in sorted(all_labels):
                ...
                plt.plot(0, 0, label = label_one.replace("_", " "), c = color_use, linestyle = line_use)
            down_coord = - (len(all_labels) // 4 + 1) * 0.1 * (len(dictio) - 1 * ("time" in dictio))
            plt.legend(ncol = 4, loc = "lower left", bbox_to_anchor = (-0.2, down_coord))
