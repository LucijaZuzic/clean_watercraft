...
for metric in metric_list:
    if test_var == "wilcoxon":
        dicti_wilcoxon_new = {"variable": [], "model": [], "ws1": [], "ws2": [], "u": [], "p": []}
        for var in var_list:
            for model in model_list:
                for ws1_ix in range(len(ws_list)):
                    ...
                    for ws2_ix in range(ws1_ix + 1, len(ws_list)):
                        ...
                        dicti_wilcoxon_new["ws1"].append(ws1)
                        dicti_wilcoxon_new["ws2"].append(ws2)
                        dicti_wilcoxon_new["u"].append(uval)
                        dicti_wilcoxon_new["p"].append(pval)
        df_write = pd.DataFrame(dicti_wilcoxon_new)
        df_write.to_csv("dicti_wilcoxon_ws_" + metric + ".csv", index = False)
