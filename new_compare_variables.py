...
for test_var in ["wilcoxon", "mann_whitney"]:
    for metric in metric_list:
        if test_var == "wilcoxon":
            dicti_wilcoxon_new = {"model": [], "ws": [], "variable1": [], "variable2": [], "u": [], "p": []}
                ...
                for ws in ws_list:
                    for var1_ix in range(len(var_list)):
                        ...
                        for var2_ix in range(var1_ix + 1, len(var_list)):
                            ...
                            dicti_wilcoxon_new["model"].append(model)
                            dicti_wilcoxon_new["ws"].append(ws)
                            dicti_wilcoxon_new["variable1"].append(var1)
                            dicti_wilcoxon_new["variable2"].append(var2)
                            dicti_wilcoxon_new["u"].append(uval)
                            dicti_wilcoxon_new["p"].append(pval)
            df_write = pd.DataFrame(dicti_wilcoxon_new)
            df_write.to_csv("dicti_wilcoxon_variables_" + metric + ".csv", index = False)
