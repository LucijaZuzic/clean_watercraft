...
for metric in metric_list:
    if test_var == "wilcoxon":
        dicti_wilcoxon_traj_new = {"model": [], "ws": [], "variable1": [], "variable2": [], "u": [], "p": []}
            ...
            for ws in ws_list:
                for var1_ix in range(len(var_list)):
                    ...
                    for var2_ix in range(var1_ix + 1, len(var_list)):
                        ...
                        try:    
                            uval, pval = stats.wilcoxon(dictio[var1][model][ws][metric], dictio[var2][model][ws][metric])
                        except:
                            uval, pval = 1.0, 1.0
                        print(metric, model, ws, var1, var2, uval, pval)
                        dicti_wilcoxon_traj_new["model"].append(model)
                        dicti_wilcoxon_traj_new["ws"].append(ws)
                        dicti_wilcoxon_traj_new["variable1"].append(var1)
                        dicti_wilcoxon_traj_new["variable2"].append(var2)
                        dicti_wilcoxon_traj_new["u"].append(uval)
                        dicti_wilcoxon_traj_new["p"].append(pval)
        df_write = pd.DataFrame(dicti_wilcoxon_traj_new)
        df_write.to_csv("dicti_wilcoxon_traj_variables_" + metric + ".csv", index = False)
    if test_var == "mann_whitney":
        dicti_mann_whitney_traj_new = {"model": [], "ws": [], "variable1": [], "variable2": [], "u": [], "p": []}
        for model in model_list:
            for ws in ws_list:
                for var1_ix in range(len(var_list)):
                    var1 = var_list[var1_ix]
                    for var2_ix in range(var1_ix + 1, len(var_list)):
                        ...
                        dicti_mann_whitney_traj_new["variable1"].append(var1)
                        dicti_mann_whitney_traj_new["variable2"].append(var2)
        ...
        df_write.to_csv("dicti_mann_whitney_traj_variables_" + metric + ".csv", index = False)