...
def plot_dict(begin_name, dict_use, save_name, use_ws = []):
    ...
    for ws in use_ws:
        ...
        for var in dict_use[ws]:
            ...
            plt.xticks([])
        ix_var += 4
    new_dir_name = "stats_new_dir_reverse/"    
...
for start_name in ["dicti_mann_whitney", "dicti_wilcoxon"]:
    ...
    df_dictio = pd.read_csv(start_name + ".csv", index_col = False)
    df_dictio_traj = pd.read_csv(start_name + "_traj.csv", index_col = False)
    ...
    for ws in set(list(df_dictio["ws"])):
        plot_dict(start_name, dicti_mann_whitney, "var_" + str(ws), [ws])
    ...
    for ws in set(list(df_dictio_traj["ws"])):
        plot_dict(start_name, dicti_mann_whitney_traj, "traj_" + str(ws), [ws])