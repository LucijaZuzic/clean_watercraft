...
for name in name_list_total:
    ...
    for metric in round_val:
        ...
        dicti_mann_whitney_var[metric] = dict()
        dicti_mann_whitney_ws[metric] = dict()
        ...
        for var in min_max_for_metric_for_ws:
            ...
            df_pd_file_var = pd.read_csv(pd_file_var, index_col = False)
            df_pd_file_ws = pd.read_csv(pd_file_ws, index_col = False)
            df_pd_file_base_metric = pd.read_csv(pd_file_base_metric, index_col = False)
            for ix_line in range(len(df_pd_file_var)):
                model_use = df_pd_file_var["model"][ix_line]
                ws_use = df_pd_file_var["ws"][ix_line]
                v1 = df_pd_file_var["variable1"][ix_line]
                v2 = df_pd_file_var["variable2"][ix_line]
                u = df_pd_file_var["u"][ix_line]
                p = df_pd_file_var["p"][ix_line]
                if model_use not in dicti_mann_whitney_var[metric]:
                    dicti_mann_whitney_var[metric][model_use] = dict()
                if ws_use not in dicti_mann_whitney_var[metric][model_use]:
                    dicti_mann_whitney_var[metric][model_use][ws_use] = dict()
                if v1 not in dicti_mann_whitney_var[metric][model_use][ws_use]:
                    dicti_mann_whitney_var[metric][model_use][ws_use][v1] = dict()
                if v2 not in dicti_mann_whitney_var[metric][model_use][ws_use]:
                    dicti_mann_whitney_var[metric][model_use][ws_use][v2] = dict()
                dicti_mann_whitney_var[metric][model_use][ws_use][v1][v2] = (u, p)
                dicti_mann_whitney_var[metric][model_use][ws_use][v2][v1] = (u, p)
            for ix_line in range(len(df_pd_file_ws)):
                var_use = df_pd_file_ws["variable"][ix_line]
                model_use = df_pd_file_ws["model"][ix_line]
                ws1 = df_pd_file_ws["ws1"][ix_line]
                ws2 = df_pd_file_ws["ws2"][ix_line]
                u = df_pd_file_ws["u"][ix_line]
                p = df_pd_file_ws["p"][ix_line]
                if var_use not in dicti_mann_whitney_ws[metric]:
                    dicti_mann_whitney_ws[metric][var_use] = dict()
                if model_use not in dicti_mann_whitney_ws[metric][var_use]:
                    dicti_mann_whitney_ws[metric][var_use][model_use] = dict()
                if ws1 not in dicti_mann_whitney_ws[metric][var_use][model_use]:
                    dicti_mann_whitney_ws[metric][var_use][model_use][ws1] = dict()
                if ws2 not in dicti_mann_whitney_ws[metric][var_use][model_use]:
                    dicti_mann_whitney_ws[metric][var_use][model_use][ws2] = dict()
                dicti_mann_whitney_ws[metric][var_use][model_use][ws1][ws2] = (u, p)
                dicti_mann_whitney_ws[metric][var_use][model_use][ws2][ws1] = (u, p)
    ...
    for metric in round_val:
        ...
        if "euclid" in metric:
            continue
        different_var[metric] = dict()
        same_var[metric] = dict()
        different_ws[metric] = dict()
        same_ws[metric] = dict()
        ...
        for var in dictio:
            if "time" in var:
                continue
            different_var[metric][var] = dict()
            same_var[metric][var] = dict()
            different_ws[metric][var] = dict()
            same_ws[metric][var] = dict()
            different_model_metric[metric][var] = dict()
            same_model_metric[metric][var] = dict()
            for model in sorted((dictio[var].keys())):
                different_var[metric][var][model] = dict()
                same_var[metric][var][model] = dict()
                different_ws[metric][var][model] = dict()
                same_ws[metric][var][model] = dict()
                ...
                for ws in sorted((dictio[var][model].keys())):
                    different_var[metric][var][model][ws]  = []
                    same_var[metric][var][model][ws]  = []
                    for v2 in dicti_mann_whitney_var[metric][model][ws][var]:
                        u, p = dicti_mann_whitney_var[metric][model][ws][var][v2]
                        len_use = len(dicti_mann_whitney_var[metric][model][ws][var])
                        num_use = (len_use * (len_use + 1)) / 2
                        if p < 0.05 / num_use:
                            different_var[metric][var][model][ws].append((v2, u, p))
                        else:
                            same_var[metric][var][model][ws].append((v2, u, p))
                    different_ws[metric][var][model][ws]  = []
                    same_ws[metric][var][model][ws]  = []
                    for ws2 in dicti_mann_whitney_ws[metric][var][model][ws]:
                        u, p = dicti_mann_whitney_ws[metric][var][model][ws][ws2]
                        len_use = len(dicti_mann_whitney_ws[metric][var][model][ws])
                        num_use = (len_use * (len_use + 1)) / 2
                        if p < 0.05 / num_use:
                            different_ws[metric][var][model][ws].append((ws2, u, p))
                        else:
                            same_ws[metric][var][model][ws].append((ws2, u, p))
        ...
        for metric in round_val:
            ...
            for model in sorted(list(model_best_for[metric][var].keys())):
                if len(model_best_for[metric][var][model]) > 0:
                    ...
                    for ws in model_best_for[metric][var][model]:
                        my_text_var += print_sentence_var(var, model, metric, ws)
                        my_text_var += print_sentence_ws(var, model, metric, ws)
        ...
        for var in sorted(var_list):
            ...
            for model in sorted(use_model):
                ...
                for ws in sorted(use_ws):
                    print_sentence_var(var, model, metric, ws)
